function [R,C,adapter]=GetData(Plot,Noise_Level)
% AUTORIGHTS
% ---------------------------------------------------------
...
C1W = R*C1C + C;
L1P1W = C1W + [-0.5;5;-2] + randn(3,1)*2;
L1P2W = C1W + [1;8;4] + randn(3,1)*2;
L1    = [L1P1W,L1P2W];
L1P1C = R'*L1P1W - R'*C;
L1P2C = R'*L1P2W - R'*C;
L1P1C = L1P1C-C1C;
...
if Noise_Level ~= 0
    u   = K*Rcamera1*(L1P1C);
    u   = u/u(3);
    L1P1C = K\(u + [randn(2,1)*Noise_Level/2; 0]);
    L1P1C = Rcamera1'*(L1P1C/norm(L1P1C));
    
    u   = K*Rcamera1*(L1P2C);
    u   = u/u(3);
    L1P2C = K\(u + [randn(2,1)*Noise_Level/2; 0]);
    L1P2C = Rcamera1'*(L1P2C/norm(L1P2C));
end
Pi1 = null([(L1P1C+C1C)', 1; (L1P2C+C1C)', 1; C1C', 1]);
...
adapter.Camera.IterPlane_L1 = Pi1;
adapter.Camera.IterPlane_L2 = Pi2;
% save the world info
adapter.World.L0.P1 = LP1W;
adapter.World.L0.P2 = LP2W;
adapter.World.L1.P1 = L1P1W;
adapter.World.L1.P2 = L1P2W;
...
if(Plot ~= 0)
    ...
    plot3(L0(1,:),L0(2,:),L0(3,:),'--b','LineWidth',2);
    plot3(C(1),C(2),C(3),'ok','LineWidth',1,'MarkerSize',5);
    text(C(1),C(2),C(3)-1,'C')
    fill3([L0(1,1),L0(1,2),C(1)],[L0(2,1),L0(2,2),C(2)],[L0(3,1),L0(3,2),C(3)], 'r', 'FaceAlpha', 0.5);
    ec1 = R'*[1,0,0]';
    ec2 = R'*[0,1,0]';
    ec3 = R'*[0,0,1]';
    ...
    fill3([L1(1,1),L1(1,2),C1W(1)],[L1(2,1),L1(2,2),C1W(2)],[L1(3,1),L1(3,2),C1W(3)], 'r', 'FaceAlpha', 0.5);
    plot3(C1W(1),C1W(2),C1W(3),'or','LineWidth',1,'MarkerSize',5);
    text(C1W(1),C1W(2),C1W(3)-1,'C1');

    MID_PL = (L1P1W+L1P2W)/2;
    DIR_L  = (L1P2W-L1P1W); DIR_L = DIR_L/norm(DIR_L);
    plot3([MID_PL(1)+depth_line*DIR_L(1),MID_PL(1)-depth_line*DIR_L(1)],...
        [MID_PL(2)+depth_line*DIR_L(2),MID_PL(2)-depth_line*DIR_L(2)],...
        [MID_PL(3)+depth_line*DIR_L(3),MID_PL(3)-depth_line*DIR_L(3)],...
        '--b','LineWidth',2);
