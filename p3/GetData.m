function [adapter,R,C,l1,l2,l3]=GetData(Plot,Noise_Level)
% AUTORIGHTS
% ---------------------------------------------------------
...
P1 =  [randn()*2.5;   randn()*5;   randn()*5];
P2  = [randn()*2.5-15;randn()*5+15;randn()*5];
P3  = [randn()*2.5-10;randn()*5-15;randn()*5];
...
if Noise_Level ~= 0
    ...
    D1C = Rcamera0'*(D1C/norm(D1C));
end

C2C = [0;5;0]+randn(3,1)*2;
C2W = R*C2C + C;
...
if Noise_Level ~= 0
    ...
    D2C = Rcamera1'*D2C/norm(D2C);
end

C3C = [2;-2;2]+randn(3,1)*2;
C3W = R*C3C + C;
D3W = P3-C3W; D3W = D3W/norm(D3W);
D3C = R'*D3W; D3C = D3C/norm(D3C);
Rcamera2 = [1,0,0;0,0,1;0,-1,0]*angle2dcm(randn()*dangle*pi/180,randn()*dangle*pi/180,randn()*dangle*pi/180);
if Noise_Level ~= 0
    u   = K*Rcamera2*D3C;
    u   = u/u(3);
    D3C = K\(u + [randn(2,1)*Noise_Level; 0]);
    D3C = Rcamera2'*(D3C/norm(D3C));
end
...
adapter.camera.c3 = C3C;
adapter.camera.d1 = D1C;
adapter.camera.d2 = D2C;
adapter.camera.d3 = D3C;
% world info
adapter.world.p1 = P1;
adapter.world.p2 = P2;
adapter.world.p3 = P3;
...
if(Plot ~= 0)
    ...
    plot3(P3(1),P3(2),P3(3),'ob','LineWidth',1,'MarkerSize',5);
    text(P3(1),P3(2),P3(3)-1,'P3')
    plot3(C3W(1),C3W(2),C3W(3),'or','LineWidth',1,'MarkerSize',5);
    text(C3W(1),C3W(2),C3W(3)-1,'C3');
    quiver3(C3W(1),C3W(2),C3W(3),3*D3W(1),3*D3W(2),3*D3W(3),'Color','r','LineWidth',2);
    plot3([C3W(1),P3(1)],[C3W(2),P3(2)],[C3W(3),P3(3)],':r');
